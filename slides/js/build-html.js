...
document.addEventListener("keydown",e=>{const k=e.key;if(k==="ArrowRight"||k===" "||k==="PageDown"){e.preventDefault();next()}if(k==="ArrowLeft"||k==="Backspace"||k==="PageUp"){e.preventDefault();prev()}if(k==="Home"){e.preventDefault();go(0)}if(k==="End"){e.preventDefault();go(N-1)}if((k==="f"||k==="F")&&!e.ctrlKey&&!e.metaKey&&!e.altKey)fs()});
